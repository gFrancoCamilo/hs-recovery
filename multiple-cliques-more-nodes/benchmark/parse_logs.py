...
def average_sync_time(start_sync, end_sync):
    ...
    for start, end in zip(start_sync, end_sync):
        print("S here is " + str(start) +" and end is " + str(end))
...
def main():
    ...
    # Print the average sync time
    print(f"Average sync time across all nodes: {avg_sync_time} seconds")
...
########################################################################################################
    ...
    plt.grid()
    plt.title('Replica 0', fontsize=16)
    ...
    plt.tight_layout()
    plt.savefig('tps_recovery-node1.pdf', dpi=300)
    plt.clf()
