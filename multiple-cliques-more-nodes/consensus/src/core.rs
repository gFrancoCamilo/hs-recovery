...
impl Core {
    ...
    #[async_recursion]
    async fn ask_faulty_parties_client(&mut self) {
        let ip = Ipv4Addr::new(127, 0, 0, 1);
        let mut port = 10000 + self.id + 3*(self.committee.authorities.len() as u64);
        let client_address = SocketAddr::new(ip.into(), port.try_into().unwrap());
        debug!("Client Socket Address: {}", client_address);

        let mut stream = TcpStream::connect(client_address).await;
        port = 10000 + self.id + 4*(self.committee.authorities.len() as u64);
        let my_addr = SocketAddr::new(std::net::IpAddr::V4(ip), port.try_into().unwrap());
        let listener = match TcpListener::bind(my_addr).await {
        Ok(listener) => listener,
        Err(e) => {
            warn!("Failed to bind listener to {}: {}", my_addr, e);
            return; // Return early if binding fails
        }
        };

        debug!("Awaiting response on {}", my_addr);

        // Blocking listener loop to accept and handle connections one at a time
        loop {
            match listener.accept().await {
                Ok((socket, peer)) => {
                    info!("Incoming connection established with {}", peer);
                    info!("Going back to process blocks");
                    break;
                }
                Err(e) => {
                    warn!("Failed to accept connection: {}", e);
                    continue; // If accept fails, continue waiting for the next connection
                }
            };
        }
    ...
    async fn process_blocks(&mut self, blocks: Blocks) -> ConsensusResult<()> {
        ...
        while !child_qc.eq(local_representative.parent()) {
            ...
            if let Some(bytes) = self.store.read(local_representative.parent().to_vec()).await.expect("Failed to read block"){
                ...
                debug!("Failed to find parent block for local representative");
                break;
            }
        }
        ...
        if local_representative.digest() <= alt_chain[left - 1].digest() {
            ...
            //self.ask_faulty_parties_client().await;
            ...
            //// Comments with two set of backslashs are new
            ...
            //let num_nodes = (self.committee.faults + 1) as usize + (self.committee.authorities.len() - self.committee.faults as usize)/2;
            ...
            debug!("Num of authorities len is {:?} and parties to kick is {:?}", num_nodes, num_parties_kick);
            parties_to_kick = &my_clique[..num_parties_kick];
            self.identified_faults = Vec::<_>::new();
            self.identified_faults.extend(parties_to_kick);
            debug!("identified faults here is {:?}", self.identified_faults.clone());
            ////}
            ...
            if num_faults as f64/(self.committee.nodes as usize - self.identified_faults.len()) as f64 >= 1 as f64/3 as f64{
                ...
                let mut current_firewall = self.round/self.network.allow_communications_at_round;
                debug!("Current firewall stage here is {:?}", current_firewall);
                ...
                debug!("Firewall after adding new stage: {:?}", self.network.firewall);
            } else {  
                let mut current_firewall = self.round/self.network.allow_communications_at_round;
            ...
            while !local_chain_tip.digest().eq(&local_representative.digest()){
                ...
                }
                {
                    ...
                    debug!("My requests here is {:?} and block sender is {:?}", my_requests.clone(), blocks.sender.clone());
                }
            }
            ...
            debug!("Value of high qc now is {:?}", self.high_qc.hash.clone());
            debug!("Value of last block hash now is {:?}", alt_chain[0].digest().clone());
            ...
            debug!("Firewall before update is {:?}", self.network.firewall.clone());
            for firewall in self.network.firewall.values_mut() {
            ...
            if num_faults as f64/(self.committee.nodes as usize - self.identified_faults.len()) as f64 >= 1 as f64/3 as f64{
                debug!("Not done yet. We still have {:?} faults, which is more than 1/3", num_faults);
                debug!("Nodes is {:?}. Iden faults is {:?}. Honest_count is {:?}", self.committee.nodes, self.identified_faults.len(), honest_count);
                // Add new firewall here
                let mut current_firewall = self.round/self.network.allow_communications_at_round;
                debug!("Current firewall stage here is {:?}", current_firewall);
                let mut stages_to_shift: Vec<u64> = self.network.firewall.keys().cloned().filter(|&stage| stage >= current_firewall).collect();
                ...
                //let mut parties_to_keep_clique: &[std::net::SocketAddr];
                let mut parties_to_keep_clique: Vec<_>;
                let num_nodes_clique = self.committee.nodes as usize;
                //let num_parties_keep_clique = self.identified_faults.len();
                //parties_to_keep_clique = &my_original_clique[..(my_original_clique.len() - num_parties_keep_clique)];
                ...
                debug!("Num to keep here: {:?}, num_parties_to_keep is {:?}, and clique size is {:?}", num_to_keep, num_parties_kick, my_original_clique);
                ...
                debug!("Clique without malicious: {:?}", parties_to_keep_clique);
                let mut my_shifted_clique = my_clique.clone();

                //self.network.firewall[&current_firewall].retain(|x| !parties_to_keep_clique.contains(x));
                let num_faults = self.committee.faults.clone();
                let my_address = self.dns[&self.committee.address(&self.name).expect("Failed to get address")];
                if let Some(firewall) = self.network.firewall.get_mut(&current_firewall){
    ...
    async fn get_faulty_parties(&mut self, sender: PublicKey) -> Vec<PublicKey> {
        ...
        //} else {
        //parties_to_keep = senders_clique.iter().rev().take(senders_clique.len() - ((self.committee.authorities.len() - self.identified_faults.len()).div_ceil(3)).try_into().unwrap()).collect::<Vec<_>>();
        ...
        debug!("len of identified faults is {:?} and parties to kick is {:?}. Num senders_clique len is {:?}", self.identified_faults.len(), num_parties_kick, senders_clique.len());
        ...
        debug!("Parties to kick here is {:?}", parties_to_kick);
        parties_to_keep = (&parties_to_keep[..num_to_keep]).to_vec();
                            //.rev()
                            //.take(num_to_keep)  // Calculates the exact number of items.
                            //.collect::<Vec<_>>();
        debug!("Parties to keep number is {:?} and are {:?}", num_to_keep, parties_to_keep.clone());
        //parties_to_kick = &my_clique[..((self.committee.authorities.len() - self.identified_faults.len()).div_ceil(3)).try_into().unwrap()];
        //}
        let mut sent_sync;
        {
    ...
    async fn handle_shifted_chain (&mut self, author: PublicKey, shifted_chain: Vec<SocketAddr>, round_number: u64) -> ConsensusResult <()> {
        ...
        if self.get_my_current_clique(author) == shift_chain_copy { 
            debug!("Clique is the same as my current. Skipping message");
            return Ok(()); 
        } 
        if author == self.name {
            return Ok(());
