...
class Committee:
    def __init__(self, names, consensus_addr, transactions_addr, mempool_addr, twins):
    ...
    def _build_consensus(self):
        ...
        return {'authorities': node, 'epoch': 1, 'num_of_twins': self.twins, 'faults': int((self.twins/2)**0.5)}
    ...
    def load(cls, filename):
        ...
        return cls(names, consensus_addr, transactions_addr, mempool_addr, data['consensus']['num_of_twins'])


class LocalCommittee(Committee):
    def __init__(self, names, twins, port):
        assert isinstance(names, list) and all(
            isinstance(x, str) for x in names)
        assert isinstance(port, int)
        assert isinstance(twins, int)
        size = len(names)
        ...
        super().__init__(names, consensus, front, mempool, twins)
