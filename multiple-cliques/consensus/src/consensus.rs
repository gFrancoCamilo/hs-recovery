...
use crate::new_helper::NewHelper;
...
lazy_static! {
    /*pub static ref REQUEST_BLOCKS: u64 = 10;*/
    /*pub static ref U32: u32 = {
        let mut REQUEST_BLOCKS = 10;
        REQUEST_BLOCKS
    };*/
    pub static ref REQUEST_BLOCKS: Arc<Mutex<u64>> = Arc::new(Mutex::new(10_u64));
...
impl Consensus {
    ...
    pub fn spawn(
        ...
        //let (tx_new_helper, rx_new_helper) = channel(CHANNEL_CAPACITY);
        ...
        //NewHelper::spawn(name, committee, store.clone(), /* rx_requests */ rx_new_helper, firewall.clone(), allow_communications_at_round, network_delay);
    }
}
...
struct ConsensusReceiverHandler {
    ...
    //tx_new_helper: Sender<(Digest, u64, PublicKey)>,
}
...
impl MessageHandler for ConsensusReceiverHandler {
    async fn dispatch(&self, writer: &mut Writer, serialized: Bytes) -> Result<(), Box<dyn Error>> {
        ...
        match bincode::deserialize(&serialized).map_err(ConsensusError::SerializationError)? {
            ...
            /*ConsensusMessage::NewSyncRequestFirst(missing, num_blocks, origin, clique) => {
                let _ = writer.send(Bytes::from("Ack")).await;
                info!("Received new sync request first {:?} for {:?} blocks from {:?}. Sender's clique is {:?}", missing.clone(), num_blocks.clone(), origin.clone(), clique.clone()); 
                {
                    let mut my_requested_cliques;
                    my_requested_cliques = REQUEST_CLIQUES.lock().unwrap();
                    if !my_requested_cliques.contains_key(&origin.clone()){
                        my_requested_cliques.insert(origin.clone(), clique.clone());
                    }
                }
                self
                    .tx_new_helper
                    .send((missing, num_blocks, origin))
                    .await
                    .expect("Failed to send consensus message")},*/
            //ConsensusMessage::NewSyncRequest(missing, num_blocks, origin) => {
            //    let _ = writer.send(Bytes::from("Ack")).await;
            //    info!("Received new sync request {:?} for {:?} blocks from {:?}", missing.clone(), num_blocks.clone(), origin.clone()); self
            //        .tx_new_helper
            //        .send((missing, num_blocks, origin))
            //        .await
            //        .expect("Failed to send consensus message")},
            ...
            /*message @ ConsensusMessage::ShiftedChain(..) => {
                debug!("Sending ack for shifted received");
                // Reply with an ACK.
                let _ = writer.send(Bytes::from("Ack")).await;
